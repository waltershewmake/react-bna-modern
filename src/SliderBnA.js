...
class SliderBnA extends PureComponent {
	...
	handleImageLoaded(e) {
		...
		if (name === "before") {
			this.state._onLoadedBefore();
		} else if (name === "after") {
			this.state._onLoadedAfter();
		}
	}
	...
	handleImageErrored(e) {
		...
		if (name === "before") {
			this.state._onError_Before(e);
		} else if (name === "after") {
			this.state._onError_After(e);
		}
	}

	componentDidMount() {
		//Perform prop checking (Developer changes)
		...
		return (
			<div id="container">
				<figure style={{ backgroundImage: `url(${before_src})` }}>
					...
					{/*Keeps re-rendering with each slider move*/}
					<Image
						after={after_src}
						before={before_src}
						alt={_alt}
						height={`${_imageHeight}vh`}
					...
					<figure
						style={{
							...
							backgroundImage: `url(${after_src})`,
							borderRight:
								_divisorBorder &&
								"1px solid rgba(255, 255, 255, 0.274)",
						}}></figure>
				</figure>
